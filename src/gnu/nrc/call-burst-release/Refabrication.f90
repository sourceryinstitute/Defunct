MODULE Refabrication
    ...
    PRIVATE
    PUBLIC :: irefab, nrefab1, nrefab2, cplrefab, vsrefab, dspgrefab, dspgwrefab, fgpavrefab, airrefab, n2refab, &
      &       arrefab, fgrefab, herefab, krrefab, xerefab, RodRefabrication, UpdateRefabGases
    
    INTEGER(ipk) :: irefab = 10000_ipk      ! Timestep to start using refabricated values (Default = 10,000)
    INTEGER(ipk) :: nrefab1 = 0_ipk         ! Lower axial node for refabrication
    INTEGER(ipk) :: nrefab2 = 0_ipk         ! Upper axial node for refabrication
    REAL(r8k) :: cplrefab = 0.0_r8k         ! Refabricated upper plenum length
    REAL(r8k) :: vsrefab = 0.0_r8k          ! Number of spring turns in refabricated upper plenum
    REAL(r8k) :: dspgrefab = 0.0_r8k        ! New plenum spring coil diameter
    REAL(r8k) :: dspgwrefab = 0.0_r8k       ! New plenum spring wire diameter
    REAL(r8k) :: fgpavrefab = 0.0_r8k       ! Fill gas pressure at time step of refabrication
    REAL(r8k) :: airrefab = 0.0_r8k         ! Fraction of air in refabricated rod
    REAL(r8k) :: n2refab = 0.0_r8k          ! Fraction of nitrogen in refabricated rod
    REAL(r8k) :: arrefab = 0.0_r8k          ! Fraction of argon in refabricated rod
    REAL(r8k) :: fgrefab = 0.0_r8k          ! Fraction of fission gas in refabricated rod
    REAL(r8k) :: herefab = 1.0_r8k          ! Fraction of helium in refabricated rod (Default = 1.0)
    REAL(r8k) :: krrefab = 0.0_r8k          ! Fraction of krypton in refabricated rod
    REAL(r8k) :: xerefab = 0.0_r8k          ! Fraction of xenon in refabricated rod
    !
    CONTAINS
    !
    SUBROUTINE RodRefabrication
    USE Conversions, ONLY : Pi, ftom, intom, PSItoMPa
    IMPLICIT NONE
    !>@brief
...
221 FORMAT(133('x'),/,48x,'Refabrication of the Rod has Occurred',/,/, &
    ...
    !
    !
    IMPURE SUBROUTINE UpdateRefabGases
    IMPLICIT NONE
    !>@brief
    ...
    USE Conversions, ONLY : ftom, mtoft
    USE Variables, ONLY : nunits, na, deltaz, qaxnorm, iq, qend, WorkArray1, WorkArray2, WorkArray3, &
      &                   ounit, naxim, jn, jpeak, avgqi, fa, ir1, jjc, nt, jmin, jmax, SI, British
    IMPLICIT NONE
    !>@brief
    ...
    !
    INTEGER(ipk) :: k, m, j, i, jnm, jt, l, jn1, jn2, j1, j2, incr
...
110             IF (xca < x(jt)) THEN
    ...
    IF (nunits == SI) WRITE (ounit,390)
    IF (nunits == British) WRITE (ounit,380)
    IF (iq == 0) THEN
        IF (nunits == SI) x1 = x1 * ftom
        jn1 = 1
        jn2 = jn(1)
        DO m = 1, jjc

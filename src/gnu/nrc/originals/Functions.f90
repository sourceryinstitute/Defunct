...
!    GENERIC :: operator(.terp.) => Linear_Interpolation
    !
    CONTAINS
    !
    PURE REAL(r8k) FUNCTION AVERAGE (VALUES)
    !>@brief
    !> This function computes the average of an array of numbers
    !>@author
    !> Ian Porter, NRC
    !>@date
    !> 6/13/2016
    ...
    AVERAGE = SUM(VALUES(:)) / UBOUND(VALUES,DIM=1)
    ...
    IF (PRESENT(nn)) THEN
        ...
        IF (PRESENT(kk)) THEN
            k = kk
        ELSE
            ! Initial guess starts at the bottom of the array
            k = 1
        END IF
    ELSE
        ...
        ! perform the linear interpolatoin. Therefore, start interpolating at the first value until
...
260 FORMAT ('***** Further messages suppressed *****')
    ...
    !
    !
    FUNCTION Step_Polate (Array, Interp_Value) RESULT (Value)
    USE Kinds
    USE Commons, ONLY : ounit
    IMPLICIT NONE
    !>@brief
    !> This function interpolates using a step-wise interpolation method
    !> It will assume that the value is equal to the lower bounds value of what's being interpolated upon
    !>@author
    !> Ian Porter, NRC
    !>@date
    !> 6/16/2016
    ...
    INTEGER(ipk) :: i
    REAL(r8k) :: Value
    REAL(r8k), PARAMETER :: Min_Delta = 1.0e-10_r8k
    REAL(r8k), INTENT(IN) :: Interp_Value
    REAL(r8k), DIMENSION(:), INTENT(IN) :: Array
    
    ! Check to see if interpolating on only 1 value
    IF (UBOUND(Array,DIM=1) == 2) THEN
        Value = Array(1)
        RETURN
    END IF
    
    ! Ensure array values are always increasing
    i = 2
    DO WHILE (i+2 <= UBOUND(Array,DIM=1))
        IF (Array(i+2) < Array(i)) THEN
            WRITE (ounit,*) 'Interpolation values are not increasing. Exeuction Terminated in Step_Polate'
            WRITE (ounit,*) Array
            ERROR STOP 'Interpolation values are not increasing. Exeuction Terminated in Step_Polate'
        END IF
        i = i + 2
    END DO
    ...
    i = 2
    Get_Value: DO
        IF (Interp_Value >= (Array(i) - Min_Delta) .AND. Interp_Value < ABS(Array(i+2) - Min_Delta)) EXIT Get_Value
        ! Incriment to check next set of pairs
        i = i + 2
        ! Assign value
        Value = Array(i-1)
        ! Ensure does not exceed array bounds. If so, keep Value equal to last provided value in array and return.
        IF (i + 1 > UBOUND(Array,DIM=1)) EXIT Get_Value
    END DO Get_Value
    
    END FUNCTION Step_Polate
    !
    !
    !
...
110     FORMAT('Error in Subroutine ShiftArray. Wrong shift type called. Program Execution Stopped.')
        ERROR STOP 'Wrong shift type called. Execution terminated in Subroutine: ShiftArray.'
    END SELECT
    !
    DEALLOCATE (DumArray)
    !
    ...
    ! Method:
    !
    ! Method of solution is by elimination using largest pivotal divisor. Each stage of elimination consists of interchanging
    ! rows when necessary to avoid division by zero or small elements. The forward solution to obtain variable n is done in
    ! n stages. The back solution for the other variables is calculated by successive substitutions. The final solution
    ! values are developed in vector b, with variable 1 in b(1), variable 2 in b(2),........, variable n in b(n).
    ! If no pivot can be found exceeding a the tolerance set by Tolerance, the matrix is considered singular and fails. 
    !
    ! ..................................................................
    !
    ...
    INTEGER(ipk), INTENT(IN) :: n
    REAL(r8k) :: biga, save
    REAL(r8k), PARAMETER :: Tolerance = 0.0_r8k
    ...
    IF (n == 0) THEN
        ...
        ERROR STOP 'Error in Subroutine: simq. N = 0.'
    END IF
    DO j = 1, n
        jy = j + 1
        jj = jj + n + 1
        ...
        IF (ABS(biga) <= Tolerance) THEN
            ! Solution failured due to singular matrix
            WRITE(ounit,20)
20          FORMAT(' Solution failure in Subroutine: simq. Singular matrix encountered.Execution stopped')
            ERROR STOP ' Solution failure in Subroutine: simq. Singular matrix encountered.Execution stopped'
        END IF
